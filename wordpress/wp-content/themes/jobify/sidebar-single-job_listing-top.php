...
if ( 'side' == esc_attr( jobify_theme_mod( 'job-display-sidebar' ) ) ) {
	return;
}
...
$count = jobify_theme_mod( 'job-display-sidebar-columns' );
	...
	<div class="col-md-<?php echo 3 == $count ? '4' : $columns; ?> col-sm-6 col-xs-12">
		<?php 
			if ( ! is_active_sidebar( 'single-job_listing-top-2' ) ) {
				the_widget( 'Jobify_Widget_Job_Categories', array( 'title' => __( 'Category', 'jobify' ) ), $args );

				if ( class_exists( 'WP_Job_Manager_Tags' ) ) {
					the_widget( 'Jobify_Widget_Job_Tags', array( 'title' => __( 'Tags', 'jobify' ) ), $args );
				}

				if ( jobify()->get( 'jetpack' ) ) {
					the_widget( 'Jobify_Widget_Job_Share', array( 'title' => __( 'Share This', 'jobify' ) ), $args );
				}
			} else {
	...
	<div class="col-md-<?php echo 3 == $count ? '5' : $columns; ?> col-sm-6 col-xs-12">
		<?php 
			if ( ! is_active_sidebar( 'single-job_listing-top-3' ) ) {
				the_widget( 'Jobify_Widget_Job_Company_Social', array( 'title' => __( 'Company Details', 'jobify' ) ), $args );
