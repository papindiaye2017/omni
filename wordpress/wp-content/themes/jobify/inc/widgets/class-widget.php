...
div[id*="job_listing"] .jobify-widget-usage.job_listing { display: none; }
div[id*="front-page"] .jobify-widget-usage.home { display: none; }
div[id*="resume"] .jobify-widget-usage.resume { display: none; }
</style>
<?php
	}
    ...
    function form( $instance ) {
        ...
        foreach ( $this->settings as $key => $setting ) {
            ...
            switch ( $setting[ 'type' ] ) {
				case 'widget-area' :
                ?>
                    <p class="jobify-widget-usage <?php echo $key; ?>"><em><?php printf( __( 'Only for use in the <strong>%s</strong> widget area.', 'marketify' ), $setting[ 'std' ] ); ?></em></p>
                <?php
                break;
                case 'description' :
                    ?>
                ...
                case 'image' :
                    wp_enqueue_media();
                    wp_enqueue_script( 'app-image-widget-admin', get_template_directory_uri() . '/js/app/admin-image-widget.js', array( 'jquery' ), '', true );
                ?>
                    <p style="margin-bottom: 0;">
                        <label for="<?php echo $this->get_field_id( $key ); ?>"><?php echo $setting[ 'label' ]; ?></label>
                    </p>
