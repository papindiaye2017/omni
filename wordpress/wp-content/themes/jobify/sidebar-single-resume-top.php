...
if ( 'side' == jobify_theme_mod( 'resume-display-sidebar' ) ) {
	return;
}
...
$count = jobify_theme_mod( 'resume-display-sidebar-columns' );
	...
	<div class="col-md-<?php echo 3 == $count ? '5' : $columns; ?> col-sm-6 col-xs-12">
		<?php	
			if ( ! is_active_sidebar( 'single-resume-top-2' ) ) {
				the_widget( 'Jobify_Widget_Job_Share', array( 'title' => __( 'Share Resume', 'jobify' ) ), $args );

				if ( get_option( 'resume_manager_enable_skills' ) ) {
					the_widget( 'Jobify_Widget_Resume_Skills', array( 'title' => __( 'Candidate Skills', 'jobify' ) ), $args );
				}

				if ( get_option( 'resume_manager_enable_resume_upload' ) ) {
					the_widget( 'Jobify_Widget_Resume_File', array( 'title' => __( 'Candidate Resume', 'jobify' ) ), $args );
				}
			} else {
	...
	<div class="col-md-<?php echo 3 == $count ? '4' : $columns; ?> col-sm-6 col-xs-12">
		<?php
			if ( ! is_active_sidebar( 'single-resume-top-3' ) ) {
				the_widget( 'Jobify_Widget_Resume_Links', array( 'title' => __( 'Candidate Details', 'jobify' ) ), $args );
